...
void amigamfm_gettitle(const unsigned int disktracks, char *title, const int titlelen)
{
  ...
  diskstore_absoluteseek(rootblock*AMIGA_DATASIZE, INTERLEAVED, disktracks);
...
void amigamfm_showinfo(const unsigned int disktracks, const int debug)
{
  ...
  for (i=0; i<amigamfm_readlong(0xc, tmpbuff); i++)
  {
    ...
    fsblock=amigamfm_readlong(0x18+(i*4), tmpbuff);
  ...
  amigamfm_decodedate(amigamfm_readlong(AMIGA_DATASIZE-0x28, tmpbuff), amigamfm_readlong(AMIGA_DATASIZE-0x24, tmpbuff), amigamfm_readlong(AMIGA_DATASIZE-0x20, tmpbuff), &tim);
  printf("Volume changed : %.2d:%.2d:%.2d %.2d/%.2d/%d\n", tim.tm_hour, tim.tm_min, tim.tm_sec, tim.tm_mday, tim.tm_mon+1, tim.tm_year+1900);

  amigamfm_decodedate(amigamfm_readlong(AMIGA_DATASIZE-0x1c, tmpbuff), amigamfm_readlong(AMIGA_DATASIZE-0x18, tmpbuff), amigamfm_readlong(AMIGA_DATASIZE-0x14, tmpbuff), &tim);
  ...
  printf("Extension : %d\n", amigamfm_readlong(AMIGA_DATASIZE-0x8, tmpbuff));
  printf("Secondary type : %d\n", amigamfm_readlong(AMIGA_DATASIZE-0x4, tmpbuff));
